...
const MyCharacter = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gray-800 text-white rounded-lg shadow-md">
      <h1 className="text-4xl font-bold mb-8 text-center">Seus Personagens</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {(characters.length <= 0 || characters == undefined) && (
          <p className="text-center mt-8 text-gray-400">Nenhum personagem encontrado.</p>
        )}
        {characters.map((el, ind) => (
          <div key={ind} className="bg-gray-700 border border-yellow-500 rounded-lg shadow-lg overflow-hidden">
            <img src={el.image} alt="Personagem" className="w-full h-40 object-cover" />
            <div className="p-4">
              <h2 className="text-xl font-bold text-yellow-400">{el.name}</h2>
              ...
              <div className="mt-4">
                <button
                  ...
                  className={`bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded ${
                    isDeleting === el.id && "opacity-50 cursor-not-allowed"
                  }`}
      ...
      {isLoading && (
        <div className="text-center mt-8">
          ...
          <p className="mt-4 text-yellow-500">Carregando personagens...</p>
        </div>
      )}
    </div>
  );
